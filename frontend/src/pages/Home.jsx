...
export default function Home() {
  ...
  const criarSorteio = async () => {
    ...
    try {
      ...
      if (!response.ok) return alert("O servidor backend recusou a criação!");
  ...
  return (
    <div className="layout-container">
      ...
      <aside className="ad-sidebar">
        <AdBlock slot="ID_ANUNCIO_ESQUERDA" />
      </aside>
                        ...
                        <div key={raffle.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '18px', backgroundColor: 'var(--secondary)', borderRadius: '10px', border: '1px solid #e0e0e0' }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: '18px' }}>
                            ...
                            <div>
                              ...
                              <div style={{ fontSize: '12px', color: 'var(--paragraph)', marginBottom: isFinished ? '4px' : '0' }}>Criado em: {dataCriacao}</div>
                              {isFinished && raffle.winner && (
                                <div style={{ fontSize: '12px', color: '#10b981', fontWeight: '700' }}>👑 Vencedor: @{raffle.winner.user} (Nº {raffle.winner.num})</div>
                              )}
                            </div>
                          </div>
                          ...
                          <div style={{ display: 'flex', alignItems: 'center', gap: '15px' }}>
                            {isFinished ? (
                              <span style={{ backgroundColor: '#fecdd3', color: '#be123c', padding: '5px 10px', borderRadius: '6px', fontSize: '11px', fontWeight: '700', letterSpacing: '1px', textTransform: 'uppercase' }}>ENCERRADO</span>
                            ) : (
                              <span style={{ backgroundColor: '#e0f2fe', color: '#0369a1', padding: '5px 10px', borderRadius: '6px', fontSize: '11px', fontWeight: '700', letterSpacing: '1px', textTransform: 'uppercase' }}>ATIVO</span>
                            )}
                            <button className="btn btn-success" style={{ width: 'auto', padding: '8px 16px', fontSize: '13px' }} onClick={() => navigate(`/admin/${raffle.id}`)}>ACESSAR</button>
                          </div>
                        </div>
                      );
                    })}
      ...
      <aside className="ad-sidebar">
        <AdBlock slot="ID_ANUNCIO_DIREITA" />
      </aside>
    </div>
  );
}